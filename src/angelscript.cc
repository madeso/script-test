...
struct AsTest : LanguageTest
{
    AsTest(AngelScript::asIScriptContext *ctx, AngelScript::asIScriptModule *mod)
        : ctx_(ctx)
    {
        rec = mod->GetFunctionByDecl("int fib_rec(int)");
        if (rec == NULL)
            Die("failed to get rec");

        loop = mod->GetFunctionByDecl("int fib_loop(int)");
        if (loop == NULL)
            Die("failed to get loop");

        str = mod->GetFunctionByDecl("int string_test(int)");
        if (str == NULL)
            Die("failed to get str");
    }
